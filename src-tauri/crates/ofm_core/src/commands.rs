...
use domain::team::{DraftStrategy, ScrimFocus, Team, TeamKind, TrainingFocus, TrainingIntensity};
...
pub fn set_lol_tactics(game: &mut Game, team_id: &str, tactics: domain::team::LolTactics) {
    if let Some(team) = game.teams.iter_mut().find(|t| t.id == team_id) {
...
pub fn set_team_roles(game: &mut Game, team_id: &str, roles: domain::team::TeamRoles) {
    if let Some(team) = game.teams.iter_mut().find(|t| t.id == team_id) {
...
pub fn select_team(game: &mut Game, team_id: &str, comp_id: &str,
                   ...
                   assembled_staff: Vec<domain::staff::Staff>,
