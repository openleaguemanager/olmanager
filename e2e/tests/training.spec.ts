...
test.describe("Training", () => {
  ...
  test("should navigate to training tab", async ({ page }) => {
    ...
    const trainingTab = page.locator('a:has-text("Training"), button:has-text("Training"), [role="tab"]:has-text("Training")');
    await trainingTab.first().click();

    // Verify training page loaded (look for training-related content)
    await expect(page.locator("text=Training").first()).toBeVisible({ timeout: 10000 });
  });
});
